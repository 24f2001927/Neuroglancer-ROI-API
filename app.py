...
def extract_and_save_roi(x: int, y: int, z: int):
    ...
    local_vol = neuroglancer.LocalVolume(
        ...
        voxel_offset=(x_min, y_min, z_min)
    )
...
def load_dataset(req: DatasetLoadRequest):
    ...
    try:
        ...
        with viewer.txn() as s:
            ...
            s.layout = '4panel'
        return {"status": "success", "message": f"Dataset loaded. Ready for extraction."}
    except Exception as e:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ng-port", type=int, default=10226, help="Port for the Neuroglancer viewer")
    parser.add_argument("--ng-token", type=str, default='roi-extractor-token', help="Token for Neuroglancer")
    parser.add_argument("--out-dir", type=str, default="./extracted", help="Directory to save output arrays")
